...
import (
	...
	"example.com/go-fee-calculator/internal/entity"
...
func (suite *OrderRepositoryTestSuite) SetupSuite() {
	...
	suite.NoError(err)
	db.Exec("CREATE TABLE orders (id varchar(255) NOT NULL, price float NOT NULL, tax float NOT NULL, final_price float)")
	suite.Db = db
}
...
func (suite *OrderRepositoryTestSuite) TestSavingOrder() {
	order, err := entity.NewOrder("123", 10.00, 1.00)
	suite.NoError(err)
